...
- 비트 연산자 & 활용
	...
	- c. % 연산 구현
		...
		- 결론
			...
			- % 대신 &를 사용하면 연산 수행이 빠름
			- a. 어떤 값에 대해 2로 나눈 나머지 값을 구하고 싶다면,
			- & 연산자를 이용하여 그 값의 하위 1비트 값만 추출
			- b. 어떤 값에 대해 4로 나눈 나머지 값을 구하고 싶다면,
...
- **switch 문법 II**
	- 상수를 좀 더 조직적으로 관리하는 방법
		- a. enum을 사용하여 상수를 정의
		- b. nested enum은 기본이 static
			- 그래서 static은 생략 가능
- **break와 continue 활용**
- continue
	- 다음 문장을 실행하지 않고 즉시 조건 검사로 이동
	- 다음 줄로 가지 않고 '변수 증가문'으로 이동
- break
...
- 6. 아규먼트 저장
- **종료**
	- JVM이 OS로 부터 받아서 쓴 모든 메모리를 OS가 회수
- **Call by reference**
- 1. 클래스 로딩
- 2. main() 호출
- 3. 로컬 변수 준비
