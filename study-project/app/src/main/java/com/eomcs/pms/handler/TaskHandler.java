...
public class TaskHandler {
  ...
  public void list() {
    ...
    for (int i = 0; i < this.size; i++) {
      System.out.printf("%d, %s, %s, %s, %s\n", this.tasks[i].no, this.tasks[i].content, this.tasks[i].deadline,
          promptStatus(this.tasks[i].status), this.tasks[i].owner);
    }
  }
  ...
  private String promptOwner(MemberHandler memberHandler, String ownerName) {
    while (true) {
      String owner = Prompt
          .inputString(String.format("담당자(%s)?(취소: 빈 문자열) ", ownerName != null ? "(" + ownerName + ")" : ""));
