...
public class App {
  ...
  void service() {
    loadData("board.data3", boardList);
    loadData("member.data3", memberList);
    loadData("project.data3", projectList);

    createMainMenu().execute();
    Prompt.close();

    saveData("board.data3", boardList);
    saveData("member.data3", memberList);
    saveData("project.data3", projectList);
  }

  @SuppressWarnings("unchecked")
  private <E> void loadData(String filepath, List<E> list) {
  ...
  private <E> void saveData(String filepath, List<E> list) {
