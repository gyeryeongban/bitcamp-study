...
- 인스턴스 메서드는 인스턴스 주소를 담고 있는 this 변수가 있음
  - inner class 를 사용 가능
- 인스턴스 필드나 메서드와 마찬가지로 this를 생략 가능
...
- 바깥 클래스의 인스턴스를 사용하는 inner 클래스라면 inner 클래스의 객체를 만드는 역할도 바깥 클래스가 하는데 유지보수에 더 나음
...
- 인스턴스 메서드는 C 인스턴스 주소를 this 변수에 저장
- 그래서 인스턴스 메서드 안에 정의된 로컬 클래스는 바깥 클래스의 인스턴스를 사용 가능
- .class 파일을 확인해보면 바깥 클래스이 인스턴스 주소를 저장하는 필드가 선언되어 있음
- 생성자에도 바깥 클래스의 인스턴스 주소를 받는 파라미터가 선언되어 있음

#### 스태틱 메서드와 로컬 클래스

- 스태틱 메서드는 C2 인스턴스 주소를 저장할 this라는 변수가 X
  - 바깥 클래스의 인스턴스를 사용 불가
...
1. 컴파일 할 때 그 값을 저장할 필드(a)를 자동으로 추가

- 사용하는 변수에 대해서만 필드를 생성
- 로컬 변수 b는 사용하지 않기 때문에 b 값을 받을 필드는 생성 X
...
#### 익명 클래스가 놓이는 장소: 스태틱 필드

- 인터페이스의 경우 static으로 선언하지 않아도 스태틱 멤버에서 사용 가능
- 스태틱 필드의 값을 준비할 때 익명 클래스를 사용 가능

#### 익명 클래스가 놓이는 장소: 인스턴스 필드

- 인터페이스의 경우 static으로 선언하지 않아도 스태틱 멤버에서 사용 가능
- 인스턴스 필드의 값을 준비할 때 익명 클래스를 사용 가능

#### 익명 클래스가 놓이는 장소: 로컬 클래스
