      ...
      for (File f : controllerFiles) {
        ...
        for (Class<?> type : interfaces) {
          if (type == PageController.class) {
            ...
            RequestMapping anno = cls.getAnnotation(RequestMapping.class);
            if (anno != null) {
              requestMapping = anno.value();
            } else {
              requestMapping = cls.getName();
            }
            requestMapping = (String) field.get(null);
  ...
  protected void service(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
      ...
      for (int i = 0; i < parameters.length; i++) {
        ...
        if (parameterType == HttpServletResponse.class || parameterType == ServletRequest.class) {
          arguments[i] = request;
